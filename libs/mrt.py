...
from struct import *
import binascii
import socket  # Needed for inet_ntop
...
class MRT:
    ...
    Author: Tim Evens
    Co-Author: Robin Röper

    The MRT utility class is used to parse MRT files.
    It supports both GZIP and BZ2 compressed MRT files.
    """
    ...
    def __init__(self, file_path):
        f = open(file_path, 'rb')
        file_header = f.read(max(len(self.GZIP_HEADER), len(self.BZ2_HEADER)))

        if file_header.startswith(self.BZ2_HEADER):
            self.f = bz2.BZ2File(file_path, 'rb')
        elif file_header.startswith(self.GZIP_HEADER):
    ...
    def parseMrtEntry(self, mrt_message, msg_len, msg_type, msg_subtype):
        try:
            ...
            if self.MRT_TYPES[msg_type] == 'TABLE_DUMP_V2':
                self.parseTableDumpV2(buf, mrt_message, msg_len, msg_type, msg_subtype)
            elif self.MRT_TYPES[msg_type] == 'BGP4MP':
    ...
    def parseTableDumpV2(self, buf, mrt_message, msg_len, msg_type, msg_subtype):
        ...
        if self.TABLE_DUMP_V2_SUBTYPES[msg_subtype] == "RIB_IPV4_UNICAST" or \
           self.TABLE_DUMP_V2_SUBTYPES[msg_subtype] == "RIB_IPV6_UNICAST":
            ...
            if self.TABLE_DUMP_V2_SUBTYPES[msg_subtype] == "RIB_IPV4_UNICAST":
                # Address family is IPv4.
                address_family = 1
                m = 4
            elif self.TABLE_DUMP_V2_SUBTYPES[msg_subtype] == "RIB_IPV6_UNICAST":
           ...
           +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
            ...
            mrt_message['seq_number'], mrt_message['prefix_len'] = unpack('!I B', buf[p:p + 5])
            p += 5
            ...
            if self.ADDRESS_FAMILY[address_family] == "IPv4":
                # Address family is IPv4.
                mrt_message['prefix'] = socket.inet_ntop(socket.AF_INET, buf[p:p+n] + b'\x00' * (m - n))
            elif self.ADDRESS_FAMILY[address_family] == "IPv6":
                # Address family is IPv6.
        ...
        elif self.TABLE_DUMP_V2_SUBTYPES[msg_subtype] == "PEER_INDEX_TABLE":
            p = 0
            """
    ...
    def parseRibEntries(self, buf, rib_entries, address_family, safi, raw_nlri):
       ...
       +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
        ...
        while p < len(buf):
            ...
            self.parseBgpAttributes(buf[p:p+rib_entry['attribute_length']],
                                     rib_entry['bgp_attribute_list'],
                                     rib_entry['raw_bgp_attributes'],
                                     rib_entry['attribute_length'],
                                     address_family,
                                     safi,
                                     raw_nlri,
                                     rib_entry['raw_mp_reach_nlri'])
    ...
    def parseBgpAttributes(self, buf, bgp_attribute_list, raw_bgp_attributes, attr_length, address_family, safi, raw_nlri, raw_mp_reach_nlri):
        ...
        while p < attr_length:
            ...
            if self.BGP_ATTRIBUTES[bgp_path_attribute['type']] == 'NEXT_HOP':
                if self.ADDRESS_FAMILY[address_family] == "IPv4":
                    # Address family is IPv4.
                ...
                raw_mp_reach_nlri['value'] = self.crateMpReachNlriAttributeRaw(
                    address_family, safi, (raw_next_hop_length + raw_next_hop), b""
                )

            # Parse out MP_REACH attribute
            elif self.BGP_ATTRIBUTES[bgp_path_attribute['type']] == 'MP_REACH_NLRI':
                """
                ...
                raw_mp_reach_nlri['value'] = self.crateMpReachNlriAttributeRaw(
                    address_family, safi, raw_next_hop_attr, b""
                )
    ...
    def parseBGP4MP(self, buf, mrt_message, msg_len, msg_type, msg_subtype):
        if (self.BGP4MP_SUBTYPES[msg_subtype] == "BGP4MP_MESSAGE_AS4" or
            self.BGP4MP_SUBTYPES[msg_subtype] == "BGP4MP_MESSAGE"):
            p = 0
            """
            BGP4MP_MESSAGE:
           ...
           +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
            ...
            if self.BGP4MP_SUBTYPES[msg_subtype] == "BGP4MP_MESSAGE_AS4":
                offset = 8
            ...
            # IPv4
            if self.ADDRESS_FAMILY[mrt_message['address_family']] == "IPv4":
                ...
                p += 4
            # IPv6
            elif self.ADDRESS_FAMILY[mrt_message['address_family']] == "IPv6":
